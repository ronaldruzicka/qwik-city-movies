import type { TvShowDetails } from '~/api/types';
...
export const HeroTvShow = component$<Props>(({ data }) => {
  const locale = use_locale();

  const image_width = 1280;
  const release_date = format_date({ date: data.first_air_date, dateStyle: 'short', locale });
  ...
  return (
    <div class="relative">
      ...
      <article class="absolute bottom-0 left-0 flex flex-col justify-end w-5/12 p-10 bg-gradient-to-r from-black/20">
        <h1 class="mb-3 text-5xl font-bold">{data.name}</h1>
        <div class="flex gap-x-2 items-center mb-5">
          <Rating value={data.vote_average} name={`rating-${data.id}`} read_only />
          <TextSubtle>
            {data.vote_count} reviews, {release_date}, {data.number_of_seasons}{' '}
